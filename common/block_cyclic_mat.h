...
{
public:
    enum fill_t {ZERO, CONSTANT, DIAGONAL, RANDOM, ARRAY};
	...
	///添加一个带array数组进行初始化的构造函数
	block_cyclic_mat_t(std::shared_ptr<blacs_grid_t> grid, blas_idx_t global_rows, blas_idx_t global_cols, double *array, blas_idx_t row_block_size = s_block_size, blas_idx_t col_block_size = s_block_size,
		fill_t fill = ZERO, double alpha = 0.0);
	...
	///  通过矩阵数组新建一个矩阵
	/// </summary>
	static std::shared_ptr<block_cyclic_mat_t>  createWithArray(std::shared_ptr<blacs_grid_t> grid, blas_idx_t global_rows, blas_idx_t global_cols, double *array);
...
private:
    ...
    blas_idx_t   m_desc[DLEN_]; //Array descriptor for dense matrices
    ...
    static const blas_idx_t s_block_size = 64;	//将矩阵行列都分成64块，一共64*64块？ TODO
};
